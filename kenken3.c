...
void print_result()
{
	...
	for (i = 1; i <= finishStatus.block_size; i++)
	{
		...
		gotoxy(x, y);
		textcolor(LIGHTBLUE);
//		setbackground((i + count_call_function) % 5 + 9);
		setbackground(i % 5 + 9);
...
//			setbackground((finishStatus.block[i][j] + count_call_function) % 5 + 9);
...
//	print_result();
	...
	while (1)
	{
		if (kbhit())
		{
			...
			{
				case 8: // delete key
					if (gameInfor.map[gameInfor.cursor.y][gameInfor.cursor.x] != 0) 
						count_fillCell--;
					gameInfor.map[gameInfor.cursor.y][gameInfor.cursor.x] = 0;
					break;
				case '0':
					return 0; // lose
				case 72:
		...
		gotoxy(get_consoleSize_cols()/2 - 8, get_consoleSize_rows()/2 + ((n - 1) * (HEIGHT + 1) + 1 + 2) / 2);
		setbackground(BLACK);
		textcolor(WHITE);
...
//	create_arr_2D(gameInfor.map, 0);
	...
	gotoxy(get_consoleSize_cols()/2 - 4, get_consoleSize_rows()/2 + ((n - 1) * (HEIGHT + 1) + 1 + 2) / 2);
	setbackground(BLACK);
	textcolor(GREEN);
	printf("Complete");
}
...
void setting()
{
	...
	{
		gotoxy(0, 0);
		printf("Map %dx%d                  %c n=%d %c", n, n, 174, n, 175);
		char c = getch();
		switch(c)
		{
			case 13:
...
int main()
{
	...
	switch(menu_process(MENU, sizeof(MENU) / sizeof(MENU[0])))
	{
		case 0: // play game;
			...
			if (play_game() == 1)
			{
				gotoxy(get_consoleSize_cols()/2 - 4, get_consoleSize_rows()/2 + ((n - 1) * (HEIGHT + 1) + 1 + 2) / 2 + 1);
				setbackground(BLACK);
